...
class CustomEnv:
    ...
    def step(self, action):
        ...
        if action == 0:  # Hold
            reward = self.net_worth * -0.1
             
        if action == 1 and self.balance > 10: # min 10 USD binance
...
if __name__ == "__main__":
    ...
    ########## TRAIN ##########
    train_df_normalized = pd.read_csv('./balanced_trend_data.csv')
    ...
    #                         'volume': 'Volume'})
    train_df_original = train_df_normalized.copy()
    ...
    agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.00001, epochs=5, optimizer=Adam, batch_size=64,
                        depth=depth, comment="penalty for holding")
